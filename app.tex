...
\begin{frame}{Motivação --- Quantização}
  ...
  \begin{itemize}
    \justifying
    \item \textit{Redução da complexidade no ínicio do reconhecimento};
...
\begin{frame}{Motivação --- Rebalanceamento de classes}
  ...
  \begin{itemize}
    ...
    \item \textit{Geração de imagens artificiais a partir do processamento das imagens da classe minoritária.}
  \end{itemize}
  \begin{figure}[htbp]
    \begin{center}
...
\begin{frame}{Experimentos --- Resultados}
  ...
  \begin{figure}
    ...
    \end{center}
    \caption{Acurácia média utilizando MSB e BIC. Os \textit{boxplots} em cinza correspondem às significâncias estatísticas com $\textit{p-value} < 0.01$ quando comparado à 256 cores. Converter para 64 cores prova ser boa escolha de processamento.}
  \end{figure}
\end{frame}
%-----------------------------------------------------------------------------
\begin{frame}{Experimentos --- Resultados}
...
\begin{frame}{Experimentos --- Resultados}
  ...
  \begin{figure}
    ...
    \end{center}
    \caption{Acurácia para os método MSB, LPP e BIC. A comparação do LPP versus MSB foi realizada com a mesma dimensionalidade. Se utilizado um número de cores correto, é possível manter ou melhorar a acurácia.}
  \end{figure}
\end{frame}
%-----------------------------------------------------------------------------
...
\begin{frame}{Experimentos --- Resultados da concatenação}
  ...
  \begin{figure}
    ...
    \end{center}
    \caption{Comparação da acurácia com o uso da projeção LPP e o método MSB para quantização das imagens com o objetivo de redução de dimensionalidade. Utilizar 64 cores é indicado como uma boa escolha do parâmetro de quantização}
  \end{figure}
\end{frame}
%-----------------------------------------------------------------------------
\begin{frame}{Experimentos --- Resultados da concatenação}
...
\begin{frame}{Geração de imagens artificiais --- Mistura limiarizada}
  ...
  \begin{figure}
    ...
    \end{center}
    \caption{Mistura de \textit{thresholds}. A imagem resultante (c) é uma composição do \textit{foreground} da primeira imagem sobre o \textit{background} da segunda.}
  \end{figure}
\end{frame}
%-----------------------------------------------------------------------------
...
\begin{frame}{Experimento --- Visualização}
  ...
  \item Analisar SMOTE ou a geração de imagens em relação à distribuição original dos dados.
  \end{itemize}
\end{frame}
%-----------------------------------------------------------------------------
